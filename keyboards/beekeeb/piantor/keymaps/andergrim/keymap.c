...
/*
 ...
 If you repeatedly tap it, the layer will be toggled on or off (like TG). It needs 5 taps by default, but you can change this by
 defining TAPPING_TOGGLE – for example, #define TAPPING_TOGGLE 2 to toggle on just two taps.
*/
...
bool process_record_user(uint16_t keycode, keyrecord_t* record) {
    switch (keycode) {
        ...
        case MK_VIM:
            if (record->event.pressed) {
                SEND_STRING(SS_TAP(X_ESCAPE) SS_LSFT(SS_TAP(X_DOT)));
            }
            return false;
